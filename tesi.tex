...
perché la struttura di $\pa{ \mathbb{R} \cup \set{\infty}, \min , +}$ non è un anello in
quanto non esiste un inverso per $\min$.

Consideriamo il prodotto $\min +$, supponiamo che le matrici $A,B$ di
cui vogliamo calcolare il prodotto siano a coefficienti interi, allora
scriviamo le matrici $A'$ e $B'$ con coefficienti
\[ a'_{i,j} = x ^{a_{i,j}} \;\;\; b'_{i,j} = x ^{b_{i,j}} \]
dove $x$ è un'indeterminata. Sia allora
\[ C' = A' \cdot B' \]
