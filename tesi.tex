...
\begin{itemize}
\item si può trovare una struttura dati che restituisca
  un'approssimazione della distanza tra due nodi in $O\pa{1}$ tempo ma
  occupando meno spazio? Vedremo questo problema nella sezione
  \ref{sec:oracoliapprossimati}
\item continuando a chiedere tempo di query $O\pa{1}$, ma richiedendo
  anche che la distanza restituita sia esatta, si può fare di meglio?
  Vedremo questo nella sezione \ref{sec:oracoliesatti}
\end{itemize}
