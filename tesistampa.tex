...
\newgeometry{inner=4cm,outer=3.5cm}
...
problemi di cammini minimi, mostrando un interessante collegamento con
il prodotto veloce tra matrici.

Nel capitolo \ref{chap:distanze} consideriamo gli algoritmi in grado
di restituire la distanza tra due punti qualsiasi del grafo. Tali
algoritmi sono chiamati oracoli in quanto, dopo un’eventuale fase di
preelaborazione dei dati, sono in grado di rispondere ad una certa
classe di richieste in tempo proporzionale alla dimensione della
risposta fornita (indipendentemente dalla dimensione del grafo).

Nel capitolo \ref{chap:oracolotutticammini} discutiamo oracoli che
restituiscono tutti i cammini fra ogni coppia di vertici in un grafo e
...
\begin{mydef}[Grafo]
  ...
  di elementi detti \textit{nodi} e $E\subseteq V \times V$ un insieme
  di relazioni dette archi.
\end{mydef}
...
Diciamo che $p$ congiunge $v_0$ a $v_k$. Dati due nodi $u,v$ chiamiamo
...
Osserviamo che la lunghezza della somma di due cammini è uguale alla
...
\item $\delta(u,z) \le \delta(u,v) + \delta(v,z)$ infatti sommando il
  ...
  cammino in $P(u,z)$
\end{itemize}
Questa distanza, però, in generale non è simmetrica, quindi definisce
...
\item dati $u,v \in V$ trovare i primi $k$ cammini più brevi in
  $P\pa{u,v}$ in tempo $O\pa{M + N\log \pa{N} + k}$;
\item una struttura (costruita in $O\pa{M+N\log \pa{n}}$ tempo) che
  dati $u,v\in V$ ritorna in ordine di lunghezza i cammini in $P(u,v)$
  estraendo l'$i$-esimo in $O\pa{\log i}$ tempo;
\item dato $s\in V$ trovare i $k$ cammini più brevi per tutte le
  destinazioni $v\in V$ in $O\pa{M + N\log \pa{N} + kN}$ tempo.
\end{itemize}
...
dell'algoritmo di Eppstein cioè $O\pa{M + N\log \pa{N} + k}$ tempo.
...
$O(N)$) con questa propriet\`a allora occupiamo $o(N^2)$ spazio per
salvarlo e sappiamo calcolare la distanza tra due vertici qualsiasi in
$O(M)$ tempo (eseguendo BFS).

Possiamo caratterizzare un $t$-spanner anche con il lemma che segue.
\begin{mylem}
...
per gli spanner che possiamo costruire:
\begin{mypro}
...
\begin{proof}
  ...
  lunghezza $h+1 < k$ da cui l'assurdo.
\end{proof}

Ci poniamo allora il problema di dire quando esistono dei $t$-spanner
di un grafo con un certo numero di archi, purtroppo si pu\`o
dimostrare il seguente teorema (cosa che noi non faremo):
\begin{myteo}
...
un $t$-spanner), potremmo comunque non poterci permettere di
elaborarlo ad ogni richiesta di distanza, per questo sono stati
sviluppati algoritmi che, dopo una preelaborazione, salvano le
informazioni sulle distanze in poco ($o(N^2)$) spazio e permettono di
ricavare la distanza approssimata tra due punti in tempo costante.
...
\subsection{Somme prefisse precalcolate: WaveletTree}
...
Wavelet Tree che occupa $2\log\pa{ 3} N + o(N)$ bit di memoria.


\subsection{Complessit\`a e ottimalità}
\label{sec:oracoloesattoottimale}

Abbiamo gi\`a visto che siamo in grado di restituire la distanza tra
due vertici arbitrari in tempo costante. Vediamo quanto spazio stiamo
occupando.
...
spazio utilizzato a $\pa{\log\pa{3} /2} N^2 + o(N^2)$ continuando a
restituire le distanze in tempo costante, ma noi non lo vedremo.
...
Abbiamo visto nell'esempio della figura \ref{fig:diamantecatena} che
il numero di cammini tra una coppia di vertici pu\`o essere
esponenziale, quindi potremmo essere interessati anche a poter
chiedere solo un certo numero $k$ di cammini, cos\`i facendo ci
avviciniamo al problema dei $k$ cammini pi\`u brevi trattato nella
sezione \ref{sec:kcammini}.
...
minimi e siano essi $p_1,p_2,...,p_c$ con $c = c(u,v)$.
...
esista $n$ tale che $u_1,...,u_n$ sono tutti distinti e che ogni nodo
in $\set{ u_{n+1},..., u_c}$ sia la ripetizione di un qualche nodo in
$\set{ u_1,..., u_n}$.
...
\begin{mypro}
  ...
  tutti e soli i cammini minimi in $P(u_i,v)$.
\end{mypro}
\begin{proof}
...
troviamo a dover esaminare tutti gli archi del grafo come si può
vedere nell'esempio di figura \ref{fig:millepiedi}.

\begin{figure}[h]
  \centering
  \includegraphics[width=1\textwidth]{millepiedi}
  \caption{Ci interessano i cammini minimi da $u$ a $v$, ma per
    trovare l'unico cammino dobbiamo esplorare tutti gli archi}
  \label{fig:millepiedi}
\end{figure}
...
Quindi, per ogni coppia di vertici, siamo interessati a sapere
qual'\`e la partizione che li separa e, in questa separazione, quali
sono i nodi separanti attraverso i quali passa un cammino minimo.
...
non dobbiamo salvare nella preelaborazione.
...
Nella sezione \ref{sec:casopessimo} vediamo che non siamo riusciti a
...
Il grafo $H$ \`e connesso\footnote{Usiamo questa nozione di
  ...
  indiretto} mentre, in generale, non lo \`e $H'$. Siano quindi $H_1,
..., H_n$ le componenti connesse di $H$.
...
Il caso in cui la nostra strategia precedente fallisce nel diminuire
lo spazio utilizzato \`e rappresentato nella figura
\ref{fig:diamante}.
...
Viste le considerazioni che abbiamo fatto sulla struttura dei cammini
minimi, questo grafo riassume la struttura dei cammini minimi fra una
coppia di vertici. Possiamo immaginare, infatti, di aver collassato
...
= O\pa{N^3 \log\pa{N}}$ bit considerando l'esempio di figura~
\ref{fig:diamantemultiplo}.
...
sullo spazio minimo che deve utilizzare al caso pessimo un oracolo per
cammini minimi.
...
Il problema di calcolare distanze e cammini minimi tra i vertici di un
grafo \`e molto studiato, viste le molte applicazioni si riducono ad
esso. Per questo abbiamo visto vari algoritmi per risolverlo nelle sue
varie sfaccettature. Anche gli oracoli per distanze, nelle due
varianti di oracoli esatti e oracoli approssimati, sono studiati e
abbiamo visto alcuni esempi di risultati disponibili in letteratura.

Abbiamo poi provato a studiare il problema di trovare un oracolo per
cammini minimi. Nel caso generale siamo riusciti a trovare un buon
algoritmo, ma \`e naturale chiedersi se \`e possibile fare di
meglio. Per tentare di rispondere a questa domanda abbiamo dato delle
indicazioni sulla possibile costruzione di un esempio in grado di
darci un \textit{lower bound} sullo spazio utilizzato da un oracolo
per cammini minimi. Siamo anche riusciti, sempre nell'intento di
costruire un limite inferiore in spazio, a ricollegarci ad un problema
di intersezione di insiemi.

Sarebbe interessante, quindi, studiare pi\`u approfonditamente il
problema dell'interesezione fra insiemi ed utilizzarne i risultati per
cercare (se possibile) di migliorare il nostro algoritmo.
